...
void gt9147_init(void)
{
    ...
    tp_init(66000000/10000);
done:
    delayms(10);
}


void tp_init(unsigned int value)
{
    EPIT2->CR = 0; //先清零
    ...
    /* 注册中断服务函数		    */
    system_register_irqhandler(EPIT2_IRQn, (system_irq_handler_t)tp_timer_irqhandler, NULL);
}
 ...
 * @return 			: 无
 */
void tp_timer_restart(unsigned int value)
{
    EPIT2->CR &= ~(1 << 0); /* 先关闭定时器 */
    EPIT2->LR = value; /* 计数值 			*/
    EPIT2->CR |= (1 << 0); /* 打开定时器 		*/
...
{
    if (EPIT2->SR & (1 << 0)) /* 判断比较事件是否发生			*/
    {
        tp_timer_stop(); /* 关闭定时器 				*/
        if (gpio_pinread(GPIO1, 9) == 0) /* KEY0 				*/
        {
            printf("%s\r\n", "touch panel intepruted");
            gt9147_read_tpcoord(); /* 读取触屏数据				*/
            for (int i = 0; i < gt9147_dev.point_num; i++)
            {
                printf("%d,%d; ", gt9147_dev.x[i], gt9147_dev.y[i]);
            }
            printf("%s", "\r\n");
        }
    }
...
void gt9147_irqhandler(void)
{
    if (gt9147_dev.initfalg == GT9147_INIT_FINISHED)
    {
        ...
        //gt9147_read_tpcoord();
        /* 开启定时器 */
        tp_timer_restart(66000000 / 10000);
    }
