...
/*
 * 重点说明！！！！base->IMR（interrupt mask register） 这个是设置是能中断的
 * base->GDIR这个设置该GPIO属于输入型还算输出型的
 * base->DR 这个就算真正的设置该GPIO上的高低电平了
 * base->ISR GPIO interrupt status register (GPIOx_ISR), GPIO 硬件给的一个状态寄存器 ISR，专门记录哪一位被出发的，这里肯定是第18位，所以要及时清零，不然就搞不清了
 */
...
{
    base->IMR &=~(1U<<pin);
    if (config->direction ==kGPIO_DigitalIntput)/* GPIO作为输入 */
    {
        base->GDIR &= ~(1U<<pin);
    }else/* 输出 */
    {
...
void gpio_intconfig(GPIO_Type *base, unsigned int pin, gpio_interrupt_mode_t pin_int_mode)
{
    ...
    uint32_t icrShift;
    icrShift = pin;
    base->EDGE_SEL &= ~(1U << pin); /*使用升高下降方式触发， 也就是ICR出发*/
    if(pin < 16) 	/* 低16位 */
    {
        icr = &(base->ICR1);
    }
...
/*
 * @description  			: 清除中断标志位(写1清除),不清它，硬件以为你还没处理完，中断会一直触发 / 进一次就卡死
